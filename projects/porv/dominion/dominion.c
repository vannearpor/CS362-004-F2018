...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (nextPlayer > (state->numPlayers - 1)){
    ...
    if(card == adventurer) {
        cardDrawn = 0;
        useAdventurer(z, drawntreasure, currentPlayer, state, cardDrawn, temphand);
    }
    
    else if(card == smithy){
        i = 0;
        useSmithy(i, currentPlayer, handPos, state);
    }
    
    else if(card == remodel){
        i = 0;
        j = 0;
        useRemodel(i, j, state, currentPlayer, handPos, choice1, choice2);
    }
  
    else if(card == village){
        useVillage(currentPlayer, handPos, state);
    }
...
int useSmithy(int i, int currentPlayer, int handPos, struct gameState *state)
{
    //+3 Cards
    for (i = 0; i < 1; i++)
    {
        drawCard(currentPlayer, state); //bug draw only one and adds +1 action
        state->numActions++;
    }
...
int useAdventurer(int z, int drawntreasure, int currentPlayer, struct gameState *state, int cardDrawn, int temphand[])
{
...
int useRemodel(int i, int j, struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2)
{
    j = state->hand[currentPlayer][choice1];  //store card we will trash
...
int useVillage(int currentPlayer, int handPos, struct gameState *state)
{
    ...
    //Bug, card does not get discarded
    return 0;
}
/* Village card effect */
...
int useGreatHall(int currentPlayer, int handPos, struct gameState *state)
{
    ...
    //+1 Actions
    state->numActions--;    //Bug -1 actions
